...
namespace dawn::native { namespace op {
    ...
    Pool2d::Pool2d(GraphBuilderBase* builder,
        ...
        if (options != nullptr && options->windowDimensionsCount != 0 && options->windowDimensions != nullptr) {
        ...
        mOptions.autoPad = options == nullptr ? wgpu::AutoPad::Explicit : options->autoPad;
        mOptions.layout = options == nullptr ? wgpu::InputOperandLayout::Nchw : options->layout;
    }
    ...
    MaybeError Pool2d::CalculateShape() {
        ...
        bool nchw = mOptions.layout == wgpu::InputOperandLayout::Nchw;
