...
namespace dawn::native { namespace op {
    Resample2d::Resample2d(GraphBuilderBase* builder,
        ...
        mOptions.mode = options == nullptr ? wgpu::InterpolationMode::NearestNeighbor : options->mode;
        if (options != nullptr && options->scalesCount != 0 && options->scales != nullptr) {
            mScales.assign(options->scales, options->scales + options->scalesCount);
        }
        if (options != nullptr && options->sizesCount != 0 && options->sizes != nullptr) {
            mSizes.assign(options->sizes, options->sizes + options->sizesCount);
        }
        if (options != nullptr && options->axesCount != 0 && options->axes != nullptr) {
